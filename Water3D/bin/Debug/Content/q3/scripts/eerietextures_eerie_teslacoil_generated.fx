// Shader: textures/eerie/teslacoil
// Generic Externs
...
// stage1's externs
uniform texture stage1_cabletest2;
uniform float3x2 stage1_tcmod;
// stage2's externs
uniform texture stage2_electricgrade3;
uniform float3x2 stage2_tcmod;
// stage3's externs
uniform texture stage3_electricslime;
...
VertexShaderOutput VertexShaderProgram(VertexShaderInput input)
{
	...
	output.stage2.uv.y += -1.2 * gameTime;
	output.stage3.uv = input.texcoords;
	output.stage3.uv.x += 1 - sin(((((input.position.x + input.position.z) * 0.001 + (1 * gameTime))) % 1025) * 6.283185) * 0.2;
	output.stage3.uv.y += 1 - sin(((((input.position.y) * 0.001 + (1 * gameTime))) % 1025) * 6.283185) * 0.2;
	output.stage3.uv.x *= 4;
	output.stage3.uv.y *= 4;
	output.stage3.uv.x += 0.5 * gameTime;
	output.stage3.uv.y += 1.5 * gameTime;
