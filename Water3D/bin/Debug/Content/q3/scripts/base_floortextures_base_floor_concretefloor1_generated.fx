// Shader: textures/base_floor/concretefloor1
// Generic Externs
...
// stage1's externs
uniform texture stage1_concretefloor1;
uniform float3x2 stage1_tcmod;
// stage2's externs
uniform texture stage2_concretefloor1;
...
VertexShaderOutput VertexShaderProgram(VertexShaderInput input)
{
	...
	output.stage2.uv.x += 1 - sin(((((input.position.x + input.position.z) * 0.001 + (0 * gameTime))) % 1025) * 6.283185) * 0.5;
	output.stage2.uv.y += 1 - sin(((((input.position.y) * 0.001 + (0 * gameTime))) % 1025) * 6.283185) * 0.5;
...
float4 PixelShaderProgram(VertexShaderOutput input) : COLOR0
{
	...
	// stage1_concretefloor1
	destination = destination * tex2D(stage1_sampler, input.stage1.uv).a;

	// stage2_concretefloor1
