// Shader: textures/sfx/proto_zzztblu3
// Generic Externs
uniform float4x4 worldViewProj;
uniform float gameTime;
// stage0's externs
uniform texture stage0_proto_zzztblu3;
uniform float3x2 stage0_tcmod;

sampler stage0_sampler = sampler_state
{
	Texture = <stage0_proto_zzztblu3>;
	minfilter = LINEAR;
	magfilter = LINEAR;
	mipfilter = NONE;
	AddressU = Wrap;
...
VertexShaderOutput VertexShaderProgram(VertexShaderInput input)
{
	...
	output.stage0.uv.x += 1 - sin(((((input.position.x + input.position.z) * 0.001 + (0.5 * gameTime))) % 1025) * 6.283185) * 0.25;
	output.stage0.uv.y += 1 - sin(((((input.position.y) * 0.001 + (0.5 * gameTime))) % 1025) * 6.283185) * 0.25;
