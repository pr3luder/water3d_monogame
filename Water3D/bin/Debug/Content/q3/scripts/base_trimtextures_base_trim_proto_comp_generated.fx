// Shader: textures/base_trim/proto_comp
// Generic Externs
uniform float4x4 worldViewProj;
uniform float gameTime;
// stage0's externs
uniform texture stage0_proto_comp2;
...
// stage3's externs
uniform texture stage3_tinfx2c;
uniform float3x2 stage3_tcmod;
// stage4's externs
uniform texture stage4_proto_comp;
uniform float3x2 stage4_tcmod;
// stage5's externs
uniform texture stage5_lightmap;
uniform float3x2 stage5_tcmod;

sampler stage0_sampler = sampler_state
{
	Texture = <stage0_proto_comp2>;
	minfilter = LINEAR;
	magfilter = LINEAR;
	mipfilter = NONE;
	AddressU = Wrap;
...
sampler stage5_sampler = sampler_state
{
	Texture = <stage5_lightmap>;
	minfilter = LINEAR;
	magfilter = LINEAR;
	mipfilter = NONE;
	AddressU = Wrap;
...
VertexShaderOutput VertexShaderProgram(VertexShaderInput input)
{
	...
	output.stage1.uv.x = s_stage1*cos(-0.5759587 * gameTime) + t_stage1 * -sin(-0.5759587 * gameTime) + (0.5 - 0.5 * cos(-0.5759587 * gameTime)+ 0.5 *sin(-0.5759587 * gameTime));
	output.stage1.uv.y = s_stage1*sin(-0.5759587 * gameTime) + t_stage1 * cos(-0.5759587 * gameTime) + (0.5 - 0.5 * sin(-0.5759587 * gameTime)- 0.5 *cos(-0.5759587 * gameTime));
	...
	output.stage2.uv.x = s_stage2*cos(-5.811946 * gameTime) + t_stage2 * -sin(-5.811946 * gameTime) + (0.5 - 0.5 * cos(-5.811946 * gameTime)+ 0.5 *sin(-5.811946 * gameTime));
	output.stage2.uv.y = s_stage2*sin(-5.811946 * gameTime) + t_stage2 * cos(-5.811946 * gameTime) + (0.5 - 0.5 * sin(-5.811946 * gameTime)- 0.5 *cos(-5.811946 * gameTime));
	// Invalid or unparsed tcMod statement.
	output.stage3.uv = input.texcoords;
	output.stage4.uv = input.texcoords;
	output.stage5.uv = input.lightmapcoords;
...
float4 PixelShaderProgram(VertexShaderOutput input) : COLOR0
{
	...
	// stage4_proto_comp
	float4 stage4_source = tex2D(stage4_sampler, input.stage4.uv);
	destination = (stage4_source * stage4_source.a) + (destination * (1.0 -stage4_source.a));

	// stage5_lightmap
	destination = tex2D(stage5_sampler, input.stage5.uv) * destination;
