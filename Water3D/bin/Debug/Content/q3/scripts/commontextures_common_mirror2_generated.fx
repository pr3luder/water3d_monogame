// Shader: textures/common/mirror2
// Generic Externs
uniform float4x4 worldViewProj;
uniform float gameTime;
// stage0's externs
uniform texture stage0_mirror1;
uniform float3x2 stage0_tcmod;
// stage1's externs
uniform texture stage1_mirror;
uniform float3x2 stage1_tcmod;

sampler stage0_sampler = sampler_state
{
	Texture = <stage0_mirror1>;
	minfilter = LINEAR;
	magfilter = LINEAR;
	mipfilter = NONE;
	AddressU = Wrap;
...
float4 PixelShaderProgram(VertexShaderOutput input) : COLOR0
{
	...
	destination = (stage0_source) + (destination * (1.0 -stage0_source.a));

	// stage1_mirror
	float4 stage1_source = tex2D(stage1_sampler, input.stage1.uv);
	destination = destination * float4(1.0 - stage1_source.r, 1.0 - stage1_source.g, 1.0 - stage1_source.b, 1.0 - stage1_source.a);
