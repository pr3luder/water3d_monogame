// Shader: textures/base_trim/panel_glo
// Generic Externs
...
// stage1's externs
uniform texture stage1_panel_glo;
...
VertexShaderOutput VertexShaderProgram(VertexShaderInput input)
{
	...
	output.stage0.uv.x += 1 - sin(((((input.position.x + input.position.z) * 0.001 + (0.06 * gameTime))) % 1025) * 6.283185) * 0.6;
	output.stage0.uv.y += 1 - sin(((((input.position.y) * 0.001 + (0.06 * gameTime))) % 1025) * 6.283185) * 0.6;
	output.stage0.uv.x *= 1.886793;
	output.stage0.uv.y *= 1.639344;
	output.stage0.uv.x += 0 * gameTime;
	output.stage0.uv.y += 4 * gameTime;
